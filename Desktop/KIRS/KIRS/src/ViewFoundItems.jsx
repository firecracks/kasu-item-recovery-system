...
function ViewFoundItems() {
  ...
  useEffect(() => {
    fetch("http://localhost:5234/found-items")
      .then((res) => res.json())
  ...
  const handleClaim = async (item) => {
    ...
    try {
      const response = await fetch("http://localhost:5234/send-claim-email-found", {
        method: "POST",
  ...
  const handleSaveStatus = async (itemId) => {
    try {
      const response = await fetch(`http://localhost:5234/found-items/${itemId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-800 to-green-600 p-6">
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl p-8 border border-gray-300 text-gray-900">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold text-gray-900">View Found Items</h2>
          <img
